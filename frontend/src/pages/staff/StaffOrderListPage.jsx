...
const StaffOrderListPage = () => {
  ...
  const verifyStripeCheckoutResult = async (orderId, sessionId, paymentFlow = "success") => {
    try {
      setIsVerifyingPayment(true);
      const { data } = await api.post(`/staff/orders/${orderId}/checkout-verify`, { sessionId });

      if (paymentFlow === "cancel") {
        setError("Stripe payment canceled");
        setSuccess("");
      } else {
        setSuccess(`Stripe payment confirmed for token #${getOrderToken(data.order)}.`);
        setError("");
      }
  ...
  useEffect(() => {
    ...
    if (payment === "cancel" && orderId && sessionId) {
      verifyStripeCheckoutResult(orderId, sessionId, "cancel");
      return;
    }
    ...
    if (payment === "success" && orderId && sessionId) {
      verifyStripeCheckoutResult(orderId, sessionId);
    }
  ...
  const moveOrderToNextStage = async (order) => {
    ...
    try {
      ...
      const { data } = await api.patch(`/staff/orders/${order._id}/status`, { status: nextStatus });
  ...
  return (
    <section className="page-section">
      ...
      {isVerifyingPayment ? <p className="success">Verifying Stripe payment...</p> : null}
